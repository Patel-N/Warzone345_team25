...
void part_1::start() {
	...
	//MapLoader* maploader = new ConquestFileReaderAdapter(new ConquestFileReader);
	string dominationmap = "./MapFiles";
	string conquestmap = "./MapFiles/ConquestFiles";
	SelectMap* selectmap = new SelectMap(dominationmap, maploader);

	selectmap->printGameMaps();
	selectmap->setMapV2(); // choose swiss.map to check the case of invalid map // spain map is showing runtime exception
	selectmap->loadmap();
